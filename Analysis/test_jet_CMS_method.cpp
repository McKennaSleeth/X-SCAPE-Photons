...
int main(int argc, char* argv[]) {
  ...
  for (const auto& filename : filenames) {
	...
	for (const auto& jet : subtracted_jets){
	  ...
	  if (sub_constituents.size() > 1){
		...
		// Place two perpendicular cones at phi = jet_phi ± 90° (± π/2)
		std::vector<double> cone_phis = {jet_phi + M_PI / 2., jet_phi - M_PI / 2.};
		for (auto& ph : cone_phis) {
		...
		for (double cone_phi : cone_phis) {
		  total_perp_cones++;

		  // Build cone center pseudojet for searching cones particles
		  double E_cone = jet_pt * std::cosh(jet_eta);
		  double Px_cone = jet_pt * std::cos(cone_phi);
		  double Py_cone = jet_pt * std::sin(cone_phi);
		  double Pz_cone = jet_pt * std::sinh(jet_eta);
		  fastjet::PseudoJet cone_center(Px_cone, Py_cone, Pz_cone, E_cone);
		  
		  // Find particles inside cone radius
		  std::vector<fastjet::PseudoJet> cone_constituents;
		  for (const auto& p : input_particles) {
		    if (deltaR(cone_center, p) <= JET_RADIUS) {
		      cone_constituents.push_back(p);
		    }
		  }

		  if (cone_constituents.empty()) continue;
		  
		  // Loop over cone pions: apply CMS flow correction and accumulate pT bins as you do for jets
		  for (const auto& c : cone_constituents) {
		    if (std::abs(c.user_index()) == 211) {  // charged pions only
		      ...
		      double delta_phi = cone_phi - Psi2;
		     ...
		     perp_cone_pion_pt_counts[binned_perp_cone_pion_pt] += static_cast<long long>(flow_weight + 0.5);
  ...
  */
  ...
  std::ofstream perp_cone_pion_pt_output_file("pion_JETSCAPE_AuAu_pTHat15_100_hard_soft_hydro10_SMASH_only_decay_100k_corrected_perpendicular_cone_pion_pt_counts.txt");
