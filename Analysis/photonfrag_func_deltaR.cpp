...
const double MIN_JET_PT = 30.0;
const double MAX_JET_PT = 40.0;
...
int main() {
    std::ifstream file("photon_pp_pTHat30_40_out_alldecayoff_prompt_1M_final_state_hadrons.dat");
    std::string line;
	  ...
	  if (smeared_jet_pt >= MIN_JET_PT && smeared_jet_pt < MAX_JET_PT){
		...
		if (constituent.user_index() == 22) {  // PID 22 is for photons
		  ...
		  if (delta_r <= JET_RADIUS){
		    ...
		    total_photons++;
		    //photon_z_values.push_back(z);
		    //std::cout << " Photon " << photon_count << ": z = " << z << std::endl;
		  }
		}
	      } 
	    }
	  }
	  //std::cout << "  Photons in jet: " << photon_count << std::endl;
	  //std::cout << "Photon z: " << photon_momentum_fraction << std::endl;
