...
-include $(PYTHIA8DIR)/Makefile.inc
...
#	$(CXX) $< -o $@ -I$(FASTJET3_INCLUDE) $(CXX_COMMON)\
	 -L$(FASTJET3_LIB) -Wl,-rpath,$(FASTJET3_LIB) -lfastjettools -lfastjet -lm\
	  $(GZIP_INC) $(GZIP_FLAGS)
#else
...
#ifeq ($(FASTJET3_USE),true)
#	$(CXX) $< -o $@ -I$(FASTJET3_INCLUDE) $(CXX_COMMON)\
	 -L$(FASTJET3_LIB) -Wl,-rpath,$(FASTJET3_LIB) -lfastjettools -lfastjet -lm -lRecursiveTools\
	  $(GZIP_INC) $(GZIP_FLAGS)
#else
#	@echo "Error: $@ requires FASTJET3"
#endif
...
PhotonTestFile PhotonFinder200: $$@.C $(PREFIX_LIB)/libpythia8.a
	$(CXX) $< $(PREFIX_LIB)/libpythia8.a -o -I$(ROOT_INCLUDE) $(CXX_COMMON)\
